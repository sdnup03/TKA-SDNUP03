...
export const StudentDashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-in slide-in-from-bottom-4 duration-500">
      ...
      {/* Greeting Section - Gen Z Style No Emoji */}
      <div className="flex flex-col gap-1 mb-2">
         <h1 className="text-3xl md:text-4xl font-black uppercase italic tracking-tighter flex items-center gap-2">
            Yo, {firstName}!
         </h1>
         <p className="font-bold text-gray-600 flex items-center gap-2 text-sm md:text-base flex-wrap">
            Siap bantai ujian bareng anak
            <span className="bg-black text-white px-2 py-0.5 border-2 border-transparent shadow-[3px_3px_0px_0px_rgba(0,0,0,0.2)] transform -rotate-2 font-black text-xs md:text-sm">
      ...
      <Card className="bg-[#4F46E5] text-white border-black shadow-[6px_6px_0px_0px_#000]">
        <h2 className="text-2xl font-black mb-2">Join Ruang Ujian</h2>
        <p className="mb-4 text-white/80 font-medium">Masukin token dari guru biar bisa akses soal.</p>
      ...
      <div className="mt-8">
          <h3 className="font-black text-xl mb-3 flex items-center gap-2">
            <span className="bg-black text-white px-2 py-0.5 text-sm">HISTORY</span> Riwayat
          </h3>
          <div className="space-y-3">
            ...
            {myAttempts.map((a, i) => {
               ...
               return (
                <Card key={`attempt-${i}`} className="flex justify-between items-center py-3 bg-white">
                    <div>
                        <div className="font-bold text-sm uppercase text-gray-500">Udah Kelar</div>
                        <div className="font-black text-lg">{exam ? exam.title : `Ujian #${a.examId}`}</div>
                    </div>
                    {isPublished ? (
                         <span className={`font-black text-xl px-3 py-1 border-2 border-black rounded ${
                             (a.score || 0) >= 75 ? 'bg-[#51CF66]' : 'bg-[#FF6B6B] text-white'
                         }`}>
                             Nilai: {a.score}
                         </span>
                    ) : (
                        <div className="bg-gray-200 border-2 border-black px-3 py-1 flex items-center gap-2 font-bold text-sm">
            ...
            {missedExams.map((ex, i) => (
                <Card key={`missed-${i}`} className="flex justify-between items-center py-3 bg-red-50 border-red-200">
                    <div>
                        <div className="font-bold text-sm uppercase text-red-500">Ujian Ditutup</div>
                        <div className="font-black text-lg opacity-60">{ex.title}</div>
                    </div>
                    <span className="font-bold text-sm text-red-600 bg-red-100 border border-red-300 px-2 py-1 rounded">
                        Skip / Ga Ngerjain
                    </span>
                </Card>
            ))}
