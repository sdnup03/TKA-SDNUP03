...
export const ExamRoom: React.FC = () => {
  const { activeExamId, exams, exitExam, submitExam, noiseLevel, updateStudentProgress } = useApp();
  ...
  }, [exam?.id, exam?.randomizeQuestions, exam?.randomizeOptions, exam?.questions]);
  ...
  useEffect(() => {
    ...
    const handleFocusOut = () => {
        // Small delay to allow focus transfer or keyboard close
        setTimeout(() => {
  ...
  useEffect(() => {
    ...
    const onVisibilityChange = () => {
        if (document.hidden) handleViolation("Terdeteksi pindah tab atau minimize browser!");
    };
    document.addEventListener("visibilitychange", onVisibilityChange);

    const onBlur = () => {
        // IGNORE BLUR IF TYPING (Mobile Keyboard Overlay often triggers window blur)
  ...
  const startExam = async () => {
    try { await document.documentElement.requestFullscreen(); } catch (e) { console.warn("FS denied", e); }
  ...
  return (
    <div className={`min-h-screen pb-20 lg:pb-24 bg-[#FDFDF7] lg:bg-gradient-to-br lg:from-[#FDFDF7] lg:to-[#F5F5F0] border-x-4 ${safeBorderColor} ${isBlurred ? 'blur-sm select-none pointer-events-none' : ''}`}>
      ...
      {/* Violation Modal */}
      <DialogOverlay isOpen={showViolationModal} onClose={() => setShowViolationModal(false)}>
         <div className="text-center">
      ...
      {/* --- HEADER --- */}
      <div className={`sticky top-0 z-30 border-b-2 border-black transition-colors shadow-md ${timeCritical ? 'bg-[#FF6B6B] text-white' : 'bg-[#FDFDF7] text-black'}`}>
        <div className="w-full px-4 lg:px-8 xl:px-12 py-3 flex justify-between items-center">
        <div className="flex items-center gap-3">
             <Button variant="ghost" size="icon" className="border-2 border-black" onClick={() => setIsNavOpen(true)}>
                 <LayoutGrid className="w-5 h-5" />
             </Button>
             <div className="hidden md:block">
                <h1 className="font-black text-lg truncate max-w-[200px]">{exam.title}</h1>
             </div>
             <div className="md:hidden font-black text-xl">
        ...
        <div className="flex items-center gap-2">
            ...
            {hasPassage && (
                <Button 
                    ...
                    size="sm" 
                    className="lg:hidden flex items-center gap-1 border-black"
                    onClick={() => setIsPassageOpen(!isPassageOpen)}
                >
                    <BookOpen className="w-4 h-4" /> <span className="text-xs">Bacaan</span>
                </Button>
            )}

            <div className={`flex items-center gap-2 px-3 py-1.5 border-2 border-black font-black text-lg shadow-[2px_2px_0px_0px_rgba(0,0,0,0.5)] ${timeCritical ? 'bg-white text-[#FF6B6B]' : 'bg-[#FFD43B] text-black'}`}>
      ...
      <div className={`fixed inset-0 z-50 transition-all duration-300 ${isNavOpen ? 'visible' : 'invisible'}`}>
              ...
              <div className="flex-1 overflow-y-auto p-4">
                  <div className="grid grid-cols-5 gap-2">
                      {processedQuestions.map((q, idx) => {
                          ...
                          if (isMarked) btnClass = 'bg-[#FFD43B] text-black'; 
                          else if (isAnswered) btnClass = 'bg-[#51CF66] text-black'; 
                          if (isCurrent) btnClass = 'bg-black text-white ring-2 ring-offset-1 ring-black';

                          return (
                              <button
                                key={q.id}
                                onClick={() => { setCurrentQuestionIndex(idx); setIsNavOpen(false); }}
                                className={`aspect-square border-2 border-black font-bold flex items-center justify-center text-sm transition-all ${btnClass}`}
                              >
                                {idx + 1}
                              </button>
                          );
      ...
      {/* --- MAIN CONTENT --- */}
      <div className={`w-full px-4 lg:px-8 xl:px-12 py-6 ${hasPassage ? 'lg:grid lg:grid-cols-[420px_1fr] lg:gap-6 xl:gap-10 lg:items-start' : 'lg:px-16 xl:px-32'}`}>
        
        {/* PASSAGE COLUMN (Desktop: Sticky Left, Mobile: Bottom Sheet) */}
        {hasPassage && (
            <div className={`
                ${isPassageOpen ? 'fixed inset-x-0 bottom-0 z-40 h-[60vh] rounded-t-2xl shadow-[0px_-4px_20px_rgba(0,0,0,0.2)] lg:static lg:h-auto lg:rounded-none lg:shadow-none lg:inset-auto' : 'hidden lg:block'}
                bg-[#FFFCEB] border-t-4 border-black lg:border-4 lg:sticky lg:top-24 lg:max-h-[calc(100vh-200px)] lg:min-h-[400px] transition-transform duration-300 animate-in slide-in-from-bottom lg:shadow-[4px_4px_0px_0px_#000]
            `}>
                <div className="p-4 h-full flex flex-col">
                    <div className="flex justify-between items-center mb-2 border-b-2 border-black/10 pb-2">
                        <div className="flex items-center gap-2">
                            <BookOpen className="w-5 h-5" />
                            <h3 className="font-black text-lg">WACANA</h3>
                        </div>
                        <Button variant="ghost" size="sm" className="lg:hidden" onClick={() => setIsPassageOpen(false)}><X /></Button>
                    </div>
                    <div 
                        className={`flex-1 overflow-y-auto ${fontSize} font-medium leading-relaxed [&>ul]:list-disc [&>ul]:pl-5 [&>ol]:list-decimal [&>ol]:pl-5`}
        ...
        {/* QUESTION AREA */}
        <Card className="min-h-[400px] flex flex-col justify-between relative animate-in fade-in lg:shadow-[8px_8px_0px_0px_#000] w-full">
             {/* Ragu-ragu Toggle */}
             <div className="absolute top-0 right-0 z-10 p-2">
                 <label className="flex items-center gap-2 cursor-pointer bg-white border-2 border-black px-2 py-1 shadow-[2px_2px_0px_0px_#000] hover:bg-yellow-50 active:translate-y-1 active:shadow-none transition-all">
                    <input 
                        type="checkbox" 
                        ...
                        className="hidden"
                    />
                    <Flag className={`w-4 h-4 ${markedForReview.has(currentQuestion.id) ? 'fill-black text-black' : 'text-gray-400'}`} />
                    <span className="text-xs font-bold uppercase select-none">Ragu</span>
                 </label>
              </div>

              <div>
                <div className="mb-4 pr-20">
                    <Badge variant="default" className="text-sm">Soal No. {currentQuestionIndex + 1}</Badge>
                </div>

                {currentQuestion.imageUrl && (
                    <div className="mb-6 group relative cursor-zoom-in inline-block" onClick={() => setLightboxImage(currentQuestion.imageUrl!)}>
                        <img 
                            src={currentQuestion.imageUrl} 
                            alt="Visual Soal" 
                            className="max-h-[250px] w-auto border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,0.1)] transition-transform group-hover:scale-[1.02]"
                        />
                        <div className="absolute bottom-2 right-2 bg-black text-white p-1 opacity-0 group-hover:opacity-100 transition-opacity">
                            <ZoomIn className="w-4 h-4" />
                        </div>
                    </div>
                )}
                
                <p className={`${fontSize} font-bold mb-6 leading-relaxed whitespace-pre-line`}>
                ...
                <div className="space-y-4">
                  ...
                  {currentQuestion.type === 'PILIHAN_GANDA' && (
                    <div className="grid gap-3">
                      {(currentQuestion._shuffledOptions || currentQuestion.options || []).map((item, idx) => {
                        ...
                        const text = isShuffled ? (item as ShuffledOption).text : (item as string);
                        ...
                        return (
                            <div 
                                ...
                                className={`
                                    flex items-start gap-3 p-4 border-2 border-black cursor-pointer transition-all active:scale-[0.98]
                                    ${isSelected ? 'bg-[#C3FAE8] shadow-[4px_4px_0px_0px_#000]' : 'bg-white hover:bg-gray-50'}
                                `}
                            >
                                <div className={`w-6 h-6 mt-0.5 border-2 border-black rounded-full flex items-center justify-center shrink-0 ${isSelected ? 'bg-black text-white' : 'bg-white'}`}>
                                    {isSelected && <div className="w-2 h-2 bg-white rounded-full" />}
                                </div>
                                <span className={`${fontSize} font-medium leading-snug`}>
                                    <span className="font-bold mr-2">{String.fromCharCode(65 + idx)}.</span>{text}
                                </span>
                            </div>
                        );
                      })}
                  ...
                  {currentQuestion.type === 'PILIHAN_GANDA_KOMPLEKS' && (
                    <div className="space-y-2">
                      <div className="mb-3 p-2 bg-yellow-50 border-2 border-yellow-400 text-xs font-bold">
                        ⚠️ Pilih <strong>SEMUA</strong> jawaban yang benar (bisa lebih dari satu)
                      </div>
                      <div className="grid gap-3">
                        {(currentQuestion._shuffledOptions || currentQuestion.options || []).map((item, idx) => {
                          const isShuffled = !!currentQuestion._shuffledOptions;
                          ...
                          return (
                              <div 
                                  ...
                                  `}
                              >
                                  <div className={`w-6 h-6 mt-0.5 border-2 border-black flex items-center justify-center shrink-0 ${isSelected ? 'bg-black text-white' : 'bg-white'}`}>
                  ...
                  {currentQuestion.type === 'BENAR_SALAH' && (
                    <div className="flex flex-col sm:flex-row gap-4">
                      <button 
                        ...
                        className={`flex-1 p-6 border-2 border-black font-black text-xl transition-all shadow-[4px_4px_0px_0px_#000] active:shadow-none active:translate-x-1 active:translate-y-1 flex items-center justify-center gap-3 ${
                          answers[currentQuestion.id] === 'true' ? 'bg-[#51CF66] text-black' : 'bg-white hover:bg-gray-50'
                        }`}
                      >
                      ...
                      <button 
                        ...
                        className={`flex-1 p-6 border-2 border-black font-black text-xl transition-all shadow-[4px_4px_0px_0px_#000] active:shadow-none active:translate-x-1 active:translate-y-1 flex items-center justify-center gap-3 ${
                          answers[currentQuestion.id] === 'false' ? 'bg-[#FF6B6B] text-white' : 'bg-white hover:bg-gray-50'
                        }`}
                      >
                  ...
                  {currentQuestion.type === 'BENAR_SALAH_TABEL' && currentQuestion.statements && (
                    <div className="bg-gray-50 p-4 border-2 border-black/10 overflow-x-auto">
                      <p className="text-sm font-bold opacity-60 mb-4 text-center">Pilih Benar atau Salah untuk setiap pernyataan</p>
                      ...
                      {(() => {
                        ...
                        return (
                          <div className="border-2 border-black bg-white">
                            {/* Table Header */}
                            <div className="grid grid-cols-[50px_1fr_auto] md:grid-cols-[60px_1fr_280px] border-b-4 border-black bg-gray-100">
                              <div className="p-3 font-black text-sm border-r-2 border-black">No</div>
                              <div className="p-3 font-black text-sm border-r-2 border-black">Pernyataan</div>
                              <div className="p-3 font-black text-sm text-center">Jawaban</div>
                            </div>
                            ...
                            {currentQuestion.statements.map((statement, idx) => {
                              ...
                              return (
                                <div 
                                  key={idx} 
                                  className={`grid grid-cols-[50px_1fr_auto] md:grid-cols-[60px_1fr_280px] border-b-2 border-black/20 ${
                                    idx % 2 === 0 ? 'bg-white' : 'bg-gray-50'
                                  }`}
                                >
                                  {/* No Column */}
                                  <div className="p-3 font-bold text-sm border-r-2 border-black/20 flex items-center justify-center">
                                    {idx + 1}
                                  </div>
                                  
                                  {/* Statement Column */}
                                  <div className="p-3 border-r-2 border-black/20 flex items-center">
                                    <span className={`${fontSize} font-medium leading-relaxed`}>
                                      {statement.text}
                                    </span>
                                  </div>
                                  
                                  {/* Answer Column */}
                                  <div className="p-2 flex flex-col md:flex-row gap-2 items-stretch md:items-center justify-center min-w-[120px] md:min-w-0">
                                    <button
                                      onClick={() => handleStatementChange(idx, 'true')}
                                      className={`px-3 py-2 border-2 border-black font-bold text-xs md:text-sm transition-all shadow-[2px_2px_0px_0px_#000] active:shadow-none active:translate-x-0.5 active:translate-y-0.5 flex items-center justify-center gap-1.5 whitespace-nowrap ${
                                        currentAnswer === 'true' 
                                          ? 'bg-[#51CF66] text-black' 
                  ...
                  {currentQuestion.type === 'MENJODOHKAN' && currentQuestion.matchingPairs && (
                    <div className="bg-gray-50 p-4 border-2 border-black/10">
                      <p className="text-sm font-bold opacity-60 mb-4 text-center">Tap Item Kiri lalu Tap Pasangan di Kanan</p>
                              ...
                              {currentQuestion.matchingPairs.map((pair, idx) => {
                                  ...
                                  return (
                                      <div 
                                        ...
                                        className={`
                                            p-3 border-2 border-black font-bold text-sm cursor-pointer transition-all min-h-[60px] flex items-center shadow-[2px_2px_0px_0px_#000]
                                            ${isActive ? 'ring-2 ring-black ring-offset-2 bg-yellow-200' : ''}
                                        `}
                                        style={{ backgroundColor: isActive ? '#fef08a' : color }}
                                      >
                                          {pair.left}
                                      </div>
                                  );
                              ...
                              {shuffledRightOptions.map((opt, rIdx) => {
                                  ...
                                  return (
                                      <div 
                                        ...
                                        className={`
                                            p-3 border-2 border-black text-sm cursor-pointer transition-all min-h-[60px] flex items-center justify-between shadow-[2px_2px_0px_0px_#000]
                                        `}
                                        style={{ backgroundColor: color }}
                                      >
                                          <span className="font-medium">{opt}</span>
                                          {isMatched && (
                                              <button onClick={(e) => { e.stopPropagation(); handleUnmatch(matchedLeftIndex); }} className="ml-2">
                  ...
                  {currentQuestion.type === 'SEQUENCING' && currentQuestion.sequenceItems && (
                    <div className="bg-gray-50 p-4 border-2 border-black/10">
                      <p className="text-sm font-bold opacity-60 mb-4 text-center">Urutkan item dengan menyeret ke posisi yang benar</p>
                      ...
                      <div className="space-y-2">
                        {(sequenceAnswers.length > 0 ? sequenceAnswers : currentQuestion.sequenceItems.map((_, i) => i)).map((itemIdx, orderIdx) => (
                          <div
                            ...
                            className="flex items-center gap-3 p-3 border-2 border-black bg-white cursor-move hover:bg-gray-50 transition-all shadow-[2px_2px_0px_0px_#000] active:shadow-none active:translate-x-0.5 active:translate-y-0.5"
                          >
                            <div className="w-10 h-10 border-2 border-black bg-[#FFD43B] flex items-center justify-center font-black text-lg shrink-0">
                              {orderIdx + 1}
                            </div>
                            <span className={`${fontSize} font-medium flex-1`}>
                              {currentQuestion.sequenceItems![itemIdx]}
                            </span>
                          </div>
                        ))}
                      </div>
                  ...
                  {currentQuestion.type === 'CLASSIFICATION' && currentQuestion.classificationItems && currentQuestion.categories && (
                    <div className="bg-gray-50 p-4 border-2 border-black/10">
                      <p className="text-sm font-bold opacity-60 mb-4 text-center">Seret item ke kategori yang sesuai</p>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                        {currentQuestion.categories.map((category, catIdx) => (
                          <div
                            ...
                            className="min-h-[150px] p-4 border-2 border-dashed border-black bg-white"
                          >
                            <div className="font-black text-lg mb-3 border-b-2 border-black pb-2">
                              {category}
                            </div>
                            <div className="space-y-2">
                              {Object.entries(classificationAnswers)
                                ...
                                .map(([itemIdx]) => (
                                  <div
                                    key={itemIdx}
                                    className="p-2 bg-[#C3FAE8] border-2 border-black font-bold text-sm"
                                  >
                                    {currentQuestion.classificationItems![parseInt(itemIdx)]}
                                  </div>
                                ))}
                            </div>
                          </div>
                        ...
                        {currentQuestion.classificationItems.map((item, itemIdx) => {
                          ...
                          return (
                            <div
                              ...
                              className={`p-3 border-2 border-black font-bold text-sm text-center cursor-move transition-all ${
                                isPlaced
                                  ? 'bg-gray-300 opacity-50 cursor-not-allowed'
                                  : 'bg-[#FFD43B] hover:bg-[#FFE066] shadow-[2px_2px_0px_0px_#000] active:shadow-none active:translate-x-0.5 active:translate-y-0.5'
                              }`}
                            >
                              {item}
                              {isPlaced && <div className="text-xs mt-1 text-gray-500">✓ Dikategorikan</div>}
                            </div>
                          );
                        })}
                      </div>
                  ...
                  {currentQuestion.type === 'ISIAN_SINGKAT' && (
                    <div className="bg-white p-2">
                        <input 
                        type="text" 
                        className="w-full text-xl font-bold border-b-4 border-black bg-transparent px-2 py-2 focus:outline-none focus:border-[#4F46E5] placeholder:italic"
       ...
       {/* --- FOOTER ACTION BAR --- */}
       <div className="fixed bottom-0 left-0 right-0 bg-white border-t-4 border-black z-30 shadow-[0px_-4px_10px_rgba(0,0,0,0.1)]">
        <div className="w-full px-4 lg:px-8 xl:px-12 py-4 flex justify-between items-center">
          <Button variant="ghost" size="sm" className="hidden md:flex items-center gap-2 opacity-50 select-none">
              <Save className="w-4 h-4" /> Auto-Saved
          </Button>

          <div className="flex gap-3 w-full md:w-auto justify-between md:justify-end">
             <Button variant="outline" onClick={() => setCurrentQuestionIndex(prev => Math.max(0, prev - 1))} disabled={currentQuestionIndex === 0} className="flex-1 md:flex-none">
                <ChevronLeft className="w-5 h-5 mr-1" /> Prev
             </Button>
             
             {isLastQuestion ? (
                 <Button variant="primary" className="flex-1 md:flex-none px-6" onClick={() => setShowConfirmSubmit(true)}>
                    Selesai <CheckCircle className="ml-2 w-5 h-5" />
                 </Button>
             ) : (
                 <Button variant="secondary" className="flex-1 md:flex-none px-6" onClick={() => setCurrentQuestionIndex(prev => Math.min(processedQuestions.length - 1, prev + 1))}>
      ...
      {/* --- CONFIRMATION MODAL --- */}
      <DialogOverlay isOpen={showConfirmSubmit} onClose={() => setShowConfirmSubmit(false)}>
        ...
        <div className="flex justify-end gap-3">
            <Button variant="outline" onClick={() => setShowConfirmSubmit(false)}>Cek Lagi</Button>
            <Button variant="primary" onClick={() => submitExam(exam.id, answers, violationCount)}>Ya, Kirim!</Button>
        </div>
      </DialogOverlay>
