...
export const TeacherDashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
      ...
      <Card className="bg-black text-white border-gray-800 shadow-[6px_6px_0px_0px_rgba(0,0,0,0.3)]">
        <div className="flex justify-between items-center mb-4">
           <h2 className="text-xl font-black flex items-center gap-2">
              <Settings className="w-6 h-6" /> Kontrol Kelas (Noise Level)
           </h2>
           <Button variant="destructive" size="sm" onClick={handleReset} title="Reset semua data">
              <RotateCcw className="w-4 h-4 mr-2" /> Reset Sistem
           </Button>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4">
          <Button 
            ...
            className={noiseLevel === 'TENANG' ? 'bg-[#51CF66] text-black border-white' : 'bg-transparent text-white border-white'}
            onClick={() => setNoiseLevel('TENANG')}
            title="Setel status kelas menjadi Tenang"
          >
            <Volume2 className="mb-1 block mx-auto" /> Tenang
          </Button>
          <Button 
            variant={noiseLevel === 'WARNING' ? 'primary' : 'outline'}
            className={noiseLevel === 'WARNING' ? 'bg-[#FFD43B] text-black border-white' : 'bg-transparent text-white border-white'}
            onClick={() => setNoiseLevel('WARNING')}
            title="Setel status kelas menjadi Warning (Perhatian)"
          >
            <AlertTriangle className="mb-1 block mx-auto" /> Warning
          </Button>
          <Button 
            variant={noiseLevel === 'BERISIK' ? 'primary' : 'outline'}
            className={noiseLevel === 'BERISIK' ? 'bg-[#FF6B6B] text-black border-white' : 'bg-transparent text-white border-white'}
            onClick={() => setNoiseLevel('BERISIK')}
            title="Setel status kelas menjadi Berisik"
          >
            <VolumeX className="mb-1 block mx-auto" /> Berisik
          </Button>
        </div>
      </Card>
      ...
      <div>
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-4">
          <h2 className="text-2xl font-black">Daftar Ujian</h2>
            ...
            {comparisonMode ? (
              ...
              <>
                <Button 
                  ...
                  className="flex items-center gap-2"
                >
                  <BarChart3 className="w-4 h-4" /> Mode Perbandingan
                </Button>
                <Button variant="primary" size="sm" onClick={handleCreate} className="flex items-center gap-2">
        ...
        <div className="overflow-x-auto bg-white border-2 border-black shadow-[4px_4px_0px_0px_#000]">
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y-2 divide-black/10">
              {exams.length === 0 ? (
                <tr>
                  <td colSpan={5} className="p-8 text-center opacity-50 font-bold">Belum ada data ujian. Silakan buat baru.</td>
                </tr>
              ) : (
                exams.map(exam => (
                  <tr key={exam.id} className={`group hover:bg-[#FDFDF7] transition-colors ${
                    comparisonMode && selectedExamsForComparison.has(exam.id) ? 'bg-blue-50' : ''
                  }`}>
                    {comparisonMode && (
                      <td className="p-4">
                        <input 
