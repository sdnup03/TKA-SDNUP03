...
import { ArrowLeft, User, TrendingUp, Award, Clock, Share2, EyeOff, FileText, Check, X, Download, FileSpreadsheet, FileText as FileTextIcon, ChevronDown } from 'lucide-react';
...
export const ExamResults: React.FC<ExamResultsProps> = ({ exam: initialExam, onBack }) => {
  ...
  const handleUpdateScore = () => {
    if (selectedAttempt) {
      ...
      if (isNaN(scoreNum) || scoreNum < 0 || scoreNum > 100) {
        setAlertMessage({ title: 'Nilai Tidak Valid', message: 'Nilai harus angka 0-100' });
        setShowAlert(true);
        return;
      }
  ...
  const exportToPDF = () => {
    ...
    doc.text(appConfig.schoolName || 'SMP PGII 1 Bandung', pageWidth / 2, 22, { align: 'center' });
    ...
    <div className="space-y-6 animate-in slide-in-from-right duration-300">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div className="flex items-center gap-4">
            ...
            <div>
            <h2 className="text-xl font-black">Hasil Ujian: {exam.title}</h2>
        ...
        <div className="flex items-center gap-2">
            <span className="text-sm font-bold mr-2 text-black">
                Status Nilai: {exam.areResultsPublished ? <span className="text-green-600 font-black">DIPUBLIKASIKAN ✅</span> : <span className="text-red-500 font-black">DISEMBUNYIKAN 🔒</span>}
            </span>
            <Button 
            ...
            >
                {exam.areResultsPublished ? <><EyeOff className="w-4 h-4" /> Sembunyikan</> : <><Share2 className="w-4 h-4" /> Umumkan Nilai</>}
            </Button>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="bg-white border-4 border-black" style={{ backgroundColor: '#FFFFFF' }}>
          <div className="flex justify-between items-start">
            <div>
              <p className="font-bold text-sm mb-1" style={{ color: '#000000', fontWeight: 'bold' }}>Total Siswa</p>
              <h3 className="text-4xl font-black" style={{ color: '#000000', fontWeight: '900' }}>{totalStudents}</h3>
            </div>
            <User className="w-8 h-8" style={{ color: '#000000', opacity: 0.7 }} />
          </div>
        </Card>
        
        <Card className="bg-[#51CF66] border-4 border-black" style={{ backgroundColor: '#51CF66' }}>
          <div className="flex justify-between items-start">
            <div>
              <p className="font-bold text-sm mb-1" style={{ color: '#000000', fontWeight: 'bold' }}>Rata-rata Nilai</p>
              <h3 className="text-4xl font-black" style={{ color: '#000000', fontWeight: '900' }}>{averageScore}</h3>
            </div>
            <TrendingUp className="w-8 h-8" style={{ color: '#000000', opacity: 0.7 }} />
          </div>
        </Card>

        <Card className="bg-[#FFD43B] border-4 border-black" style={{ backgroundColor: '#FFD43B' }}>
          <div className="flex justify-between items-start">
            <div>
              <p className="font-bold text-sm mb-1" style={{ color: '#000000', fontWeight: 'bold' }}>Nilai Tertinggi</p>
              <h3 className="text-4xl font-black" style={{ color: '#000000', fontWeight: '900' }}>{highestScore}</h3>
            </div>
            <Award className="w-8 h-8" style={{ color: '#000000', opacity: 0.7 }} />
          </div>
        </Card>
      </div>
                 ...
                 <div className="flex-1 overflow-y-auto pr-2 space-y-4 mb-4">
                    {exam.questions.map((q, idx) => (
                        <div key={q.id} className="border-2 border-black p-3 bg-white shadow-[2px_2px_0px_0px_#ccc]">
                            ...
                            <p className="text-sm font-bold mb-2 text-black" style={{ opacity: 0.9 }}>{q.text}</p>
