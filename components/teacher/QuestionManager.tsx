...
import { Exam, Question, QuestionType, QuestionBankItem } from '../../types';
import { Button, Input, Textarea, Card, Badge, ConfirmDialog, AlertDialog } from '../ui/brutalist';
import { RichTextEditor } from '../ui/RichTextEditor';
import { Plus, Trash2, ArrowLeft, X, BookOpen, Edit, Image as ImageIcon, Loader2, Save, Database, Filter, Search, Download, History, ChevronLeft, ChevronRight, ChevronsLeft, ChevronsRight } from 'lucide-react';
...
export const QuestionManager: React.FC<QuestionManagerProps> = ({ exam, onUpdateExam, onBack }) => {
  ...
  const handleSaveQuestion = async () => {
    ...
    try {
      ...
      if (newQType === 'PILIHAN_GANDA' || newQType === 'PILIHAN_GANDA_KOMPLEKS') {
        ...
        const hasEmptyOption = newQOptions.some(o => !o.trim());
        if (hasEmptyOption) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="sticky top-0 z-50 bg-white border-b-4 border-black shadow-[4px_4px_0px_0px_#000] -mx-6 px-6 py-4 mb-4">
        <div className="flex items-center justify-between gap-4">
          <div className="flex items-center gap-4">
            <Button variant="outline" size="icon" onClick={onBack}><ArrowLeft /></Button>
            <div>
              <h2 className="text-xl font-black">Kelola Soal: {exam.title}</h2>
      ...
      <div className="space-y-4">
        {questions.map((q, idx) => (
          <Card key={q.id} className={`relative group transition-all ${editingQId === q.id ? 'border-[#4F46E5] ring-2 ring-[#4F46E5] ring-offset-2' : ''}`}>
            <div className="absolute top-2 right-2 flex gap-2">
              <Button 
                variant="outline" 
                size="icon" 
                className="h-8 w-8 bg-white hover:bg-yellow-100" 
            ...
            <p className="font-bold mb-2 pr-20">{q.text}</p>
            
            {q.type === 'PILIHAN_GANDA' && q.options && (
              <ul className="list-disc pl-5 text-sm space-y-1">
                {q.options.map((opt, i) => (
                  <li key={i} className={i.toString() === q.correctKey ? "font-bold text-green-700 underline" : ""}>
                    {opt}
                  </li>
                ))}
              </ul>
            ...
            {q.type === 'PILIHAN_GANDA_KOMPLEKS' && q.options && (
              <ul className="list-disc pl-5 text-sm space-y-1">
                {q.options.map((opt, i) => {
                  ...
                  return (
                    <li key={i} className={isCorrect ? "font-bold text-green-700 underline" : ""}>
                      {opt} {isCorrect && "✓"}
                    </li>
                  );
                })}
            ...
            {q.type === 'MENJODOHKAN' && q.matchingPairs && (
              <div className="mt-2 grid gap-1 text-sm bg-gray-50 p-2 border-2 border-black/10">
                {q.matchingPairs.map((pair, i) => (
                  <div key={i} className="flex justify-between border-b border-gray-300 last:border-0 pb-1">
                    <span>{pair.left}</span>
                              ...
                              {showBankPreview === bankQ.id && (
                                <div className="mt-3 p-4 bg-white border-2 border-black text-sm space-y-3">
                                  ...
                                  <div>
                                    ...
                                    <p className="text-sm">{bankQ.text}</p>
                                  </div>

                                  {bankQ.passage && (
                                    <div className="bg-yellow-50 p-3 border-2 border-yellow-400">
                                      <p className="font-bold text-xs mb-1">📖 Wacana:</p>
                                      <div className="text-xs max-h-40 overflow-y-auto" dangerouslySetInnerHTML={{ __html: bankQ.passage }} />
                                    </div>
                                  )}
                                    ...
                                    <div>
                                      ...
                                      <div className="space-y-1">
                                        {bankQ.options.map((opt, i) => {
                                          ...
                                          return (
                                            <div key={i} className={`p-2 border-2 text-xs ${isCorrect ? 'bg-[#C3FAE8] border-[#51CF66]' : 'bg-white border-gray-300'}`}>
                                    ...
                                    <div>
                                      ...
                                      <div className="grid grid-cols-2 gap-2 text-xs">
                                        {bankQ.matchingPairs.map((pair, i) => (
                                          <div key={i} className="p-2 border-2 border-black bg-gray-50">
                                            <span className="font-bold">{pair.left}</span> → <span>{pair.right}</span>
                                          </div>
                                        ))}
                                      </div>
                                    </div>
                                      ...
                                      <div className="space-y-1 text-xs">
                                        {bankQ.statements.map((stmt, i) => (
                                          <div key={i} className="p-2 border-2 border-black bg-white">
                                            <p className="mb-1">{i + 1}. {stmt.text}</p>
                                  ...
                                  {bankQ.classificationItems && bankQ.classificationItems.length > 0 && (
                                    <div>
                                      ...
                                      <div className="text-xs">
                                        <p><strong>Items:</strong> {bankQ.classificationItems.join(', ')}</p>
                                        <p><strong>Categories:</strong> {bankQ.categories?.join(', ')}</p>
                                      </div>
                                    </div>
                                  )}
