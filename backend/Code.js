...
function doPost(e) {
  ...
  try {
    ...
    try {
      ...
      return responseJSON({ success: false, message: 'Invalid request format' });
    }
    ...
    const action = sanitizeString(params.action || '', 50);
...
function saveImageToDrive(base64Data, fileName) {
  try {
    const FOLDER_NAME = "IPA_EXAM_ASSETS";
  ...
  sheetNames.forEach(name => {
    ...
    if (!firstRowIsHeader) {
      ...
      if (!headersMatch && missingHeaders.length === 0) {
        ...
        // Note: We don't reorder headers automatically to avoid breaking existing data
        // Missing headers are added, but order is preserved for existing headers
...
function saveExam(examData) {
  ...
  // Map data according to current headers
  const rowData = headers.map(key => {
    if (key === 'questions') {
      // Ensure questions is always an array and stringify it
      return JSON.stringify(examData.questions || []);
    }
...
/**
 * Update question in QuestionBank
 */
...
function analyzeDistractors(question, allAttempts, topGroup, bottomGroup) {
  ...
  for (var i = 0; i < question.options.length; i++) {
    ...
    analysis.push({
      optionIndex: i,
      optionText: question.options[i].substring(0, 50),
...
/**
 * Update question bank with analysis metrics
 */
