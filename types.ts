...
export interface Question {
  ...
  matchingPairs?: { left: string; right: string }[]; // For Matching: Left side prompt, Right side answer
  statements?: { text: string; correctAnswer: 'true' | 'false' }[]; // For BENAR_SALAH_TABEL: Array of statements with correct answers
  // For SEQUENCING: items to order and correct sequence
  sequenceItems?: string[]; // Items to be ordered
  correctSequence?: number[]; // Array of indices representing correct order
  // For CLASSIFICATION: items to categorize
  classificationItems?: string[]; // Items to be classified
